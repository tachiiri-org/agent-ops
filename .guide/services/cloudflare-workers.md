...
Cloudflare Workers runtime for repositories and services that run BFF, gateway, or adapter workloads on Workers.
...
- Keep provider-agnostic semantics in `../domains/foundations.md` and `../domains/*.md`.
- Keep role responsibilities in `../roles/`.
- Use this file only for Workers-specific constraints.
